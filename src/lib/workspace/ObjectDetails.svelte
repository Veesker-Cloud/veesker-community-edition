...
<script lang="ts">
  ...
  type Tab = "overview" | "columns" | "indexes" | "related" | "dataflow" | "vectors" | "details" | "vision";
  ...
  // Auto-load vector indexes when the vectors tab is active for a TABLE/VIEW
  // and we haven't fetched yet. Side-effecting state mutation belongs in an
  // $effect, not in a template `{@const}` (Svelte 5's state_unsafe_mutation
  // guard fires when you mutate $state inside derived/template paths).
  $effect(() => {
    if (
      activeTab === "vectors" &&
      vectorIndexes.kind === "idle" &&
  ...
  const tabs = $derived.by((): Array<{ id: Tab; label: string; count?: number }> => {
    ...
    return [{ id: "dataflow", label: "Graph" }, { id: "vision" as Tab, label: "◉ Vision" }];
  });
        ...
        <div class="vec-panel">
          ...
          <div class="vec-results">
            ...
            {:else if searchResult.kind === "ok"}
              ...
              {/if}
            {:else}
              <!-- Idle: $effect above auto-fires loadVectorIndexes when tab activates -->
              {#if vectorIndexes.kind === "loading" || vectorIndexes.kind === "idle"}
                <div class="loading-row"><span class="spinner"></span> Loading…</div>
