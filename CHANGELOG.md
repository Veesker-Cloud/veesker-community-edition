...
- **BUG-2 — PLAN tab empty + "Visual Flow failed: ORA-00904: FILTER_PREDICATES" after Explain Plan.** Users whose schema contained a stale `PLAN_TABLE` (pre-10g DDL or manually created without `ACCESS_PREDICATES`/`FILTER_PREDICATES` columns) hit ORA-00904 on every explain execution. A schema-local `PLAN_TABLE` silently shadows the `PUBLIC` synonym. Added a nested try/catch for `errorNum === 904` in `explainPlan()` — on ORA-00904 the query retries with `NULL AS ACCESS_PREDICATES, NULL AS FILTER_PREDICATES`; the plan tree renders normally, predicates are informational only.
